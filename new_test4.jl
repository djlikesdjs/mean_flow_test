...
const ν1 = 1000
const ν2 = 1e-5
@inline variable_viscosity(x, y, z, t) = ν1*(erf((x-(Lx/4))/(Lx/16))+erf((-x-(Lx/4))/(Lx/16))+2)
@inline buoyancy_stabiliser(x,y,z,t,b)=(z*Ni²-b)*ν2*(erf((x-(Lx/4))/(Lx/16))+erf((-x-(Lx/4))/(Lx/16))+2)
...
model = HydrostaticFreeSurfaceModel(grid = grid,
                                    ...
                                    forcing = (b=buoyancy_forcing, v = mean_forcing,))
